...
import {sendOTPVerificationEmail} from "./send-email.js"
...
mongoose.connect("mongodb+srv://root:akhilakhi815007@emergency.06wcw.mongodb.net/emergency?retryWr" +
...
const userSchema = new mongoose.Schema({
    ...
    email: {
        type: String,
        required: [
            true, 'Please enter Email Address'
        ],
        unique: true,
        lowercase: true
    },
...
const UserTrash = new mongoose.model("UserTrash", userTSchema)

//nodemailer syntax new user sign up
...
//Reading the count of services and users//
...
app.put('/updateService', (req, res) => {
    ...
    //    let id;     console.log(email) Service.findOne({email: email}, (err, user)
...
//Read operations Reading users details
...
//Restoring details from trash Service trash
app.post('/restoreService', async(req, res) => {

    const {email} = req.body
    ServiceTrash.findOne({
        email: email
    }, (err, user) => {
        const {
            sname,
...
//Delete operations delete user from database
...
// //Email verification function   let transporter =
// nodemailer.createTransport({     service: 'SendPulse'     ,     auth: {
//   user:"19131547@gptcthirurangadi.in",         pass:"akhilakhi815007"    },
// }) const sendOTPVerificationEmail = async ({ email},res) => {     try{
//  const otp =`${Math.floor(1000 + Math.random() * 9000)}`         //mail
// options         // const mailOptions ={         //
// from:"19131547@gmail.com",         //     to:email,         //     html:
// `<p>Enter ${otp} in the app to verify your email address.</p>         //
// <p>This code expires in 1 hour</>`         // };         const saltRounds =
// 10;         const hashedOTP = await bcrypt.hash(otp, saltRounds);
// res.send({message: hashedOTP})         // const newOTP = await new otpModel({
//         //     usersEmail: email,         //     otp: hashedOTP,         //
//   createdAt: Date.now()         // });         //save data         // await
// newOTP.save();         // await transporter.sendMail(mailOptions);         //
